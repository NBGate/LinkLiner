...
void GameLayer::showWin()
{
    ...
    this->addChild(label);
    this->scheduleOnce(schedule_selector(GameLayer::exitGame), 2.0);
}
...
bool GameLayer::canClearTwo(cocos2d::CCPoint pointpre, cocos2d::CCPoint pointcurrent)
{
	bool bMatch = false;
...
bool GameLayer::ccTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
	...
	if (ptouch.x > 250 && ptouch.y > 420) {
        ...
        this->schedule(schedule_selector(GameLayer::exitGame), 0);
        return true;
	}
